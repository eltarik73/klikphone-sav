...
def client_step2():
    ...
    # Grille de marques avec vrais logos À CÔTÉ du nom
    cols = st.columns(2)
    for i, m in enumerate(marques):
        with cols[i % 2]:
            logo_url = BRAND_LOGOS.get(m, "")
            
            # Utiliser des colonnes pour logo + bouton côte à côte
            if logo_url and m != "Autre":
                col_logo, col_btn = st.columns([0.2, 0.8])
                with col_logo:
                    st.markdown(f"""
                    <div style="display: flex; align-items: center; justify-content: center; height: 42px; background: #f8fafc; border-radius: 10px; padding: 6px;">
                ...
                # Pour "Autre" - bouton normal avec emoji
                if st.button(f"🔧 {m}" if m == "Autre" else m, key=f"brand_{m}", use_container_width=True):
                    st.session_state.data["marque"] = m
                    st.session_state.step = 3
                    st.rerun()
